...
(function ($) {
  ...
  $.fn.datePicker = function (option) {
    ...
    (function ($input, newOption) {
      ...
      if (newOption.inline === true) {
        $stage.addClass(newOption.cssClass.main).append(drawCalendar($input, newOption, val, val));
...
//                var momentForPrint = moment(display).add(-Math.floor((newOption.monthCount - 1) / 2), 'M');
        ...
        tr.find('>td:first table tr td.' + newOption.cssClass.prev).html(newOption.inline ? '' : newOption.prev).click(function () {
          if (!newOption.inline) {
        ...
        tr.find('>td:last table tr td.' + newOption.cssClass.next).html(newOption.inline ? '' : newOption.next).click(function () {
          if (!newOption.inline) {
            $(this).parents('.' + newOption.cssClass.main).empty().append(
                drawCalendar($input, newOption, selected, momentDisplay.add(1, 'M'))
            );
        ...
        return tag('table').addClass(newOption.inline ? newOption.cssClass.wrapper + " inline" : newOption.cssClass.wrapper).append(tr);
      }
      ...
      function drawMonth($input, newOption, selected, display) {
        ...
        var $table = tag('table').addClass(newOption.cssClass.month).attr('data-date', momentDisplay.format('YYYY-MM-DD')).css(
            {borderCollapse: 'collapse'}).append(
            tag('tr')
            .append(tag('td').addClass(newOption.cssClass.prev))
            .append(tag('td').addClass(newOption.cssClass.yearMonth).attr('colspan', '5').css({textAlign: 'center'}).text(
                momentDisplay.format('YYYY-MM')))
        ...
        if (newOption.monthCount == 1 && newOption.changeMonthYear) {
          ...
          $selectYear.change(function () {
            $(this).parents('.' + newOption.cssClass.main).empty().append(
                drawCalendar($input, newOption, selected, momentDisplay.year($(this).val())));
          });
          ...
          $selectMonth.change(function () {
            $(this).parents('.' + newOption.cssClass.main).empty().append(
                drawCalendar($input, newOption, selected, momentDisplay.month(parseInt($(this).val()))));
          });
        ...
        //noinspection JSDuplicatedDeclaration
        for (var i = 0; i < 7; i++) {
          $trForWeekHeader.append(
              tag('th').addClass(newOption.cssClass.weekdayName).addClass(newOption.cssClass.weekday[i]).text(newOption.weekdayName[i]));
        }
